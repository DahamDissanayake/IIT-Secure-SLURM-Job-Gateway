#!/usr/bin/env bash
# redeploy-slurm-deck.sh — Update the single canonical clone at /opt/slurm-deck.
# Run as slurmadmin on the login node.
#
# Open-source deploy model (M03 §A): ONE git clone at /opt/slurm-deck IS the live
...
# Override with SD_GPU_SOURCE=<git-remote-or-path> to deploy from elsewhere.
set -euo pipefail
...
if [ -z "${SD_REEXEC:-}" ]; then
    _self_copy="$(mktemp "${TMPDIR:-/tmp}/redeploy-slurm-deck.XXXXXX")"
    cat "$0" > "$_self_copy"
    export SD_REEXEC=1
...
INSTALL="${SD_GPU_HOME:-/opt/slurm-deck}"
BRANCH="${SD_GPU_BRANCH:-main}"
...
SOURCE="${SD_GPU_SOURCE:-/home/slurmadmin/slurm-deck}"
...
if [ -f "$INSTALL/deploy/slurm-deck-mailer" ]; then
    step "Syncing slurm-deck-mailer to /usr/local/bin ..."
    sudo cp "$INSTALL/deploy/slurm-deck-mailer" /usr/local/bin/slurm-deck-mailer
    sudo chmod 755 /usr/local/bin/slurm-deck-mailer
    ok "slurm-deck-mailer updated"
fi

# Sync the pod-notify checker (fired by slurm-deck-pod-notify.timer). NOTE: the
# .service/.timer unit files themselves are NOT installed here -- that is a
# manual one-time step (see deploy/slurm-deck-pod-notify.service header),
# matching how sudoers-gateway-admin is also a manual install, not something
# this script pushes on every deploy.
if [ -f "$INSTALL/deploy/slurm-deck-pod-notify" ]; then
    step "Syncing slurm-deck-pod-notify to /usr/local/bin ..."
    sudo cp "$INSTALL/deploy/slurm-deck-pod-notify" /usr/local/bin/slurm-deck-pod-notify
    sudo chmod 755 /usr/local/bin/slurm-deck-pod-notify
    ok "slurm-deck-pod-notify updated"
fi
...
for _s in slurm-deck-adduser slurm-deck-deluser; do
...
if systemctl list-unit-files 2>/dev/null | grep -q '^slurm-deck-audit'; then
    step "Restarting slurm-deck-audit ..."
    _pid_before=$(systemctl show slurm-deck-audit --property=MainPID --value 2>/dev/null || echo 0)
    _ts_before=$(systemctl show slurm-deck-audit --property=ExecMainStartTimestamp --value 2>/dev/null || echo "")
    sudo systemctl restart slurm-deck-audit || fail "systemctl restart slurm-deck-audit failed"
    sleep 1
    _pid_after=$(systemctl show slurm-deck-audit --property=MainPID --value 2>/dev/null || echo 0)
    _ts_after=$(systemctl show slurm-deck-audit --property=ExecMainStartTimestamp --value 2>/dev/null || echo "")
    if [ "$_pid_after" = "0" ] || ! systemctl is-active --quiet slurm-deck-audit; then
        fail "slurm-deck-audit failed to start after restart — check: journalctl -u slurm-deck-audit -n 20"
    fi
    if [ "$_pid_after" = "$_pid_before" ] && [ "$_ts_after" = "$_ts_before" ]; then
        fail "slurm-deck-audit PID unchanged after restart (${_pid_after}) — daemon did not restart. Check sudoers and journalctl."
    fi
