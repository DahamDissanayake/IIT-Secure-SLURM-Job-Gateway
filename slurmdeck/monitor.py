...
from slurmdeck.config import is_admin, load_config
from slurmdeck.slurm import (cancel, hold, release, requeue, queue,
                          job_detail, job_efficiency, filtered_history)
from slurmdeck.ui import (BACK_TO_MAIN, STYLE, console, err, info, kv, ok,
...
def monitor_menu() -> None:
    while True:
        ...
        if choice == "View my queue":
            ...
            from slurmdeck.dashboard import run_hardware_stats
            run_hardware_stats()
...
def show_history() -> None:
    ...
    from slurmdeck.config import load_config, jobs_dir, is_admin
    from rich.table import Table
    cfg = load_config()
    screen("Job History")
