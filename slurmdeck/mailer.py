...
def _daemon_mail(to: str, subject: str, html: str,
    ...
    try:
        from slurmdeck.auditclient import daemon_request
        ...
        <tr><td bgcolor="#FFFFFF" style="background:#FFFFFF;padding:4px 32px 28px">
          ...
          <div style="margin-top:24px">
            ...
            <ol style="margin:0;padding-left:20px;color:#374151;font-size:13px;line-height:2.2">
              ...
              <li>The Slurm Deck interface will launch automatically after that.</li>
            </ol>
          </div>
        ...
        <tr><td bgcolor="#F4F4F5" style="background:#F4F4F5;padding:18px 32px;border-top:1px solid #E4E4E7">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td align="right" style="color:#A1A1AA;font-size:11px;font-family:monospace">slurm-deck</td>
            </tr>
...
def send_jupyter_warning(
    ...
    accent  = "#F59E0B"
    tip_cmd = f"ssh -p {gateway_port} {to.split('@')[0] if '@' in to else 'you'}@{gateway_host}" if gateway_host else "slurm-deck"
    html = f"""<!DOCTYPE html>
  ...
  <tr><td bgcolor="#FFFFFF" style="background:#FFFFFF;padding:28px 32px">
    ...
    <div style="margin-top:20px;border-left:3px solid {accent};padding:14px 18px;background:#FFFBEB">
      ...
      <p style="margin:0;color:#78350F;font-size:13px;line-height:1.7">
        Open <strong>slurm-deck</strong> → <strong>Live Monitor</strong>, select your job, and press <strong>E</strong> to add 2 more hours.
...
def send_pod_available_notice(username: str, email: str, free: int, total: int,
    ...
    user asked for (slurmdeck.pod_notify.check_and_notify). Sent by the
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%">
  ...
  <tr><td bgcolor="#FFFFFF" style="background:#FFFFFF;padding:28px 32px">
    <div style="border-left:3px solid {accent};padding:14px 18px;background:#F0FDF4">
      ...
      <p style="margin:0;color:#166534;font-size:13px;line-height:1.7">This is a single notification — you will not be emailed again unless you subscribe from the Main Menu once more. Log in to <strong>slurm-deck</strong> and submit your job before the pods fill back up.</p>
    </div>
  </td></tr>
  <tr><td bgcolor="#F4F4F5" style="background:#F4F4F5;padding:18px 32px;border-top:1px solid #E4E4E7">
    <table width="100%" cellpadding="0" cellspacing="0">
