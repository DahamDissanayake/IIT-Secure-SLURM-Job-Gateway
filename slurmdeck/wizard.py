...
from slurmdeck.config import load_config, jobs_dir, user_dir
from slurmdeck.jobs import (JobSpec, build_interactive_cmd,
                         gpu_share_note, make_job_folder, pip_install_block,
                         render_notebook_sbatch, render_sbatch, resource_defaults)
from slurmdeck.launchspec import (LaunchSpec, apply_pods, default_spec, from_rerun,
                               from_template, recent_scripts, to_job_spec)
from slurmdeck.review import run_hub
from slurmdeck.slurm import submit_job, get_node_stats
from slurmdeck.ui import STYLE, err, info, kv, ok, panel, screen, select_menu, warn
from slurmdeck.validate import clean_run_command, in_jail, safe_listdir
...
def _validate_and_show_errors(script_text: str, username: str, cfg) -> bool:
    ...
    from slurmdeck.validate import validate_sbatch
    from slurmdeck.ui import err as _err
...
def _tier3_own_script(username: str, cfg) -> str | None:
    ...
    import questionary
    from slurmdeck.validate import in_user_browse_jail, user_browse_roots, in_jail
    from slurmdeck.ui import info, err as _err
    from slurmdeck import auditclient
...
def _current_share_note(gpu_shards: int) -> str:
    ...
    from slurmdeck.pods import pod_count, pod_count_known
...
def _post_submit_notebook(job_id: str, folder: str) -> None:
    ...
    """
    import time
    from slurmdeck.connect import parse_connect, render_card, wait_ready
    from slurmdeck.slurm import queue as _q
    from slurmdeck.ui import console as _con
...
def _run_own_sbatch(cfg, user: str, jdir: str) -> None:
    ...
    if success:
        ...
        try:
            from slurmdeck.dashboard import run_dashboard
...
def run_wizard(prefill: dict | None = None) -> None:  # noqa: C901 (one flow, read top to bottom)
    ...
    from slurmdeck.config import is_admin
    ...
    while ls is None:
        ...
        from slurmdeck.templates import pick_template
    ...
    if ls.intent == "notebook":
        ...
        (Path(folder) / ".sd-jupyter").write_text("")  # marks this job as a JupyterLab session
