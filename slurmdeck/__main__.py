...
def _do_password_change(username: str) -> bool:
    ...
    import subprocess
    from slurmdeck.ui import warn, info, ok as _ok
    ...
    try:
        ...
        if r.returncode == 0:
            try:
                from slurmdeck import daemonclient, auditclient
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="slurm-deck")
    ...
    def _fire_login_notification() -> None:
        try:
            from slurmdeck import daemonclient, mailer
    ...
    try:
        if not args.no_splash:
            from slurmdeck.splash import show_splash, show_status_block
            show_splash()
            show_status_block()
        from slurmdeck.menu import run_menu
        run_menu()
    except Exception as exc:
        from slurmdeck import auditclient as _ac
