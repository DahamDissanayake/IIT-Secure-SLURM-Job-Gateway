...
from slurmdeck.config import Config, conda_sh, jobs_dir, load_config
from slurmdeck import slurm as _slurm
from slurmdeck.slurm import submit_job
from slurmdeck.ui import BACK_TO_MAIN, STYLE, err, info, kv, ok, screen, select_menu, warn
from slurmdeck.validate import in_jail, safe_listdir
...
def _run_env_setup(cfg: Config) -> None:
    ...
    from slurmdeck.envbuilder import FRAMEWORK_LABELS, build_env
    ...
    if success:
        ...
        from slurmdeck.envs import EnvEntry, _save_venv_registry, _load_venv_registry
...
def _run_smoke_test(cfg: Config) -> None:
    screen("Smoke Test")
    from slurmdeck.envs import list_all_envs
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".sh", dir=cfg.nfs_root, delete=False, prefix="slurmdeck_smoke_"
    ) as tmp:
        tmp.write(script)
        tmp_path = tmp.name
...
def _run_model_download(cfg: Config) -> None:
    ...
    from slurmdeck.models import model_menu
    model_menu(cfg)
...
def _run_install_prebuilt(cfg: Config) -> None:
    ...
    from slurmdeck.envbuilder import (
...
def _run_env_manager(cfg: Config) -> None:
    from slurmdeck.envs import env_manager
    env_manager(cfg)
