...
from slurmdeck import auditclient
from slurmdeck.config import load_config, is_admin
...
def provision_user(username: str, admin: bool = False,
    ...
    cmd = ["sudo", "-n", "/usr/local/bin/slurm-deck-adduser", username]
    ...
    if email and ok_pw:
        from slurmdeck import mailer as _mailer
...
def offboard_user(username: str, purge: bool = False) -> tuple[bool, str]:
    ...
    cmd = ["sudo", "-n", "/usr/local/bin/slurm-deck-deluser", username]
    if purge:
...
# mail.send handler, AND the SLURM-job MailProg (slurm-deck-mailer). Toggling it here
# stops/restores ALL outbound email immediately.

def is_mail_disabled() -> bool:
    from slurmdeck.mailer import mail_disabled
...
def set_mail_disabled(set_by: str) -> tuple[bool, str]:
    import json
    import os
    from slurmdeck.mailer import _mail_flag_path
    try:
        p = _mail_flag_path()
        with open(p, "w") as f:
...
def resize_pod_count(new_n: int, node: str = "iit-MS-7E06") -> tuple[bool, str]:
    ...
    from slurmdeck.pods import fits_new_pod_count
    ...
    rc, out, err = _run(
        ["sudo", "-n", "-u", "slurmadmin",
         "/opt/slurm-deck/deploy/resize-pods.sh", str(new_n)], timeout=120)
...
def _pods_menu(style, node: str = "iit-MS-7E06") -> None:
    ...
    from slurmdeck.ui import console, ok, err, screen
...
def _qos_menu(style) -> None:
    import questionary
    from rich.table import Table
    from slurmdeck.ui import console, info, ok, err, screen, select_menu, warn
...
def _provision_menu(style) -> None:
    import questionary
    from slurmdeck.ui import ok, err, info, warn
    ...
    from slurmdeck.config import load_config, user_dir
    _cfg = load_config()
...
def _view_audit_log(style) -> None:
    import questionary
    from slurmdeck.ui import console, screen, info
...
def _view_users(style) -> None:
    import questionary
    from rich.table import Table
    from slurmdeck.ui import console, info, screen, warn
...
def _view_job_output(style) -> None:
    import questionary
    from slurmdeck.ui import console, info, err, screen, select_menu
...
def _view_service_health(style) -> None:
    import questionary
    from slurmdeck.ui import console, info, screen, select_menu

    _UNITS = ["slurm-deck-audit", "slurmctld", "slurmd", "mariadb", "slurmdbd"]
...
def _maintenance_menu(style) -> None:
    import os
    import questionary
    from slurmdeck.ui import info, ok, err, screen, select_menu
...
def _mail_service_menu(style) -> None:
    import questionary
    from slurmdeck.ui import screen, info, ok, err, warn
    screen("Mail Service")
...
def _login_as_menu(style) -> None:
    ...
    import questionary
    from slurmdeck.ui import info, ok, err, screen, select_menu, warn
    ...
    try:
        subprocess.run(
            ["sudo", "-H", "-u", target, "/usr/local/bin/slurm-deck"])
...
def admin_menu() -> None:
    ...
    from rich.table import Table
    from slurmdeck.ui import STYLE, console, info, ok, err, screen, select_menu, warn
    ...
    while True:
        ...
        if choice == "Drain node":
            ...
            from slurmdeck.config import jobs_dir
            from slurmdeck.slurm import filtered_history
            cfg = load_config()
            t = Table(show_header=True, header_style="bold cyan")
            ...
            from slurmdeck.accounting import usage_by_user
            t = Table(show_header=True, header_style="bold cyan")
