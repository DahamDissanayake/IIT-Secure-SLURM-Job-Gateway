...
from slurmdeck.slurm import (NodeStats, QueueEntry, cancel, extend_job_time, get_node_stats,
                          queue, filtered_history, _effective_user)
from slurmdeck.ui import console, err, ok
...
def _is_jupyter_job(job_id: str, jdir: str) -> bool:
    """True when the job folder contains the .sd-jupyter marker file."""
    log_path = _find_job_log(job_id, jdir)
    if log_path is None:
        return False
    return (Path(log_path).parent / ".sd-jupyter").exists()


def _is_ready_job(job_id: str, jdir: str) -> bool:
    """True when the job's folder contains the .sd-ready marker."""
    log_path = _find_job_log(job_id, jdir)
    if log_path is None:
        return False
    return (Path(log_path).parent / ".sd-ready").exists()
...
def _connect_renderable(sel: QueueEntry | None, current_user: str, jdir: str):
    ...
    if not _is_jupyter_job(sel.job_id, str(Path(jdir) / sel.user)):
        return None
    from slurmdeck.connect import parse_connect, render_card
...
def _build_hw_panel(stats: NodeStats | None) -> Panel:
    ...
    if stats:
        ...
        alloc_parts = [
            f"GPU {stats.shard_alloc}/{stats.shard_total} pods"
...
def run_dashboard(job_id: str | None = None) -> None:
    ...
    from slurmdeck.config import is_admin as _is_admin_fn
    ...
    def _refresh_data() -> None:
        ...
        for _j in jobs:
            ...
            if (_jfolder / ".sd-jupyter").exists():
                _new_jready[_j.job_id] = (_jfolder / ".sd-ready").exists()
        _jready = _new_jready
        ...
        if sel and is_own and sel.job_id:
            ...
            if (remaining is not None
                ...
                try:
                    from slurmdeck import daemonclient as _dc
                    from slurmdeck import mailer as _mailer
                    _email = _dc.email_for(current_user)
                    if _email:
                        from slurmdeck.config import load_config as _lcfg
                        _cfg2 = _lcfg()
            ...
            while True:
                ...
                if key == "q":
                    ...
                    if sel and sel.user != current_user and not _admin:
                        ...
                        live.stop()
                        import questionary
                        from slurmdeck.ui import STYLE
                        ...
                        if questionary.confirm(
                            ...
                            if success:
                                from slurmdeck import auditclient as _audit
                    ...
                    if sel and sel.user == current_user and _is_jupyter[0] and sel.state == "RUNNING":
                        ...
                        if success:
                            try:
                                from slurmdeck import daemonclient as _dc2
                                from slurmdeck import mailer as _m2
                                _email2 = _dc2.email_for(current_user)
                                if _email2:
                                    from slurmdeck.slurm import show_job as _sj
                                ...
                                pass
                            from slurmdeck import auditclient as _audit2
