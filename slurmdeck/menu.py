...
from slurmdeck.ui import BACK_TO_MAIN, STYLE, info, screen, select_menu
...
def run_menu() -> None:
    from slurmdeck.config import load_config as _lc, is_admin as _ia
    _admin = _ia(_lc())
    while True:
        # Maintenance banner
        _maint = None
        try:
            from slurmdeck.admin import get_maintenance
            _maint = get_maintenance()
        except Exception:
            pass
        if _maint:
            from slurmdeck.ui import console
        ...
        if choice is None:
            ...
            from slurmdeck.wizard import run_wizard
            run_wizard()

        elif choice.startswith("2."):
            from slurmdeck.workspace import run_workspace
            run_workspace()
            ...
            from slurmdeck.admin import admin_menu
            admin_menu()
...
def _notify_menu() -> None:
    ...
    from slurmdeck.ui import ok, err, screen
    ...
    from slurmdeck import pod_notify
...
def _jobs_status() -> str:
    ...
    try:
        from slurmdeck.slurm import get_node_stats, queue
        import getpass
...
def _jobs_menu() -> None:
    from slurmdeck.dashboard import run_dashboard, run_hardware_stats
    from slurmdeck.monitor import (show_queue, manage_job, browse_and_tail_log,
    ...
    while True:
        ...
        if choice is None:
            ...
            from slurmdeck.accounting import usage_menu
            usage_menu()
        elif choice == "My running services":
            from slurmdeck.notebooks import services_menu
            services_menu()
...
def _settings_menu() -> None:
    from slurmdeck.config import load_config as _lc
    _cfg = _lc()
    ...
    while True:
        ...
        if choice is None:
            ...
            from slurmdeck.setup import _run_env_setup
            _run_env_setup(_cfg)
        elif choice == "Install prebuilt environment":
            from slurmdeck.setup import _run_install_prebuilt
            _run_install_prebuilt(_cfg)
        elif choice == "Run smoke test":
            from slurmdeck.setup import _run_smoke_test
            _run_smoke_test(_cfg)
        elif choice == "Advanced SLURM shell":
            from slurmdeck.shell import run_shell
            run_shell()


def _show_cluster_status() -> None:
    from slurmdeck.slurm import get_partitions
    from slurmdeck.ui import console, warn
