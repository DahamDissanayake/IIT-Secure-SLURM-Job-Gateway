...
def make_job_folder(jobs_dir: str, spec: JobSpec) -> str:
    ...
    # deploy/slurm-deck-adduser.sh / deploy/fix-shared-perms.sh), and mkdir
    ...
    # deploy/slurm-deck-adduser.sh, as a second, independent layer — it's what
...
_NODE_ADDR_SNIPPET = [
    'SD_NODE_ADDR=$(scontrol show node "${SLURMD_NODENAME:-$(hostname -s)}" 2>/dev/null'
    ' | sed -n "s/.*NodeAddr=\\([^ ]*\\).*/\\1/p")',
    '[ -z "$SD_NODE_ADDR" ] && SD_NODE_ADDR=$(hostname -I | awk "{print \\$1}")',
    '[ -z "$SD_NODE_ADDR" ] && SD_NODE_ADDR=127.0.0.1',
    "",
]


# Bash that resolves a free TCP port into $SD_PORT before anything is printed.
#
...
def _free_port_snippet(base_port: int) -> list[str]:
    ...
    return [
        ...
        "PYEOF",
        ")",
        '[ -z "$SD_PORT" ] && { echo "ERROR: no free port on this node — try again." >&2; exit 1; }',
        "",
    ]


# Bash that defines $SD_USER_ROOT and exposes the shared assets inside it.
#
...
def _user_home_snippet() -> list[str]:
    lines = ['SD_USER_ROOT="/shared/users/$USER"', 'mkdir -p "$SD_USER_ROOT"']
    for asset in ("models", "envs", "data", "datasets"):
        target = f'"$SD_USER_ROOT/{asset}"'
        lines += [
            f'_sd_target={target}',
            f'if [ -L "$_sd_target" ] || [ ! -e "$_sd_target" ]; then',
            f'    ln -sfn /shared/{asset} "$_sd_target"',
            "else",
            f'    echo "WARNING: $_sd_target already exists and is not a symlink -- skipping the shared {asset} asset (it will not be reachable at that path)." >&2',
            "fi",
        ]
    lines.append("")
    return lines
...
def _ready_marker_snippet(folder: str) -> list[str]:
    return [
        ...
        '    if (exec 3<>"/dev/tcp/$SD_NODE_ADDR/$SD_PORT") 2>/dev/null; then',
        f'        touch "{folder}/.sd-ready"; break',
        "    fi",
        "    sleep 2",
        "  done ) &",
        "",
...
def render_notebook_sbatch(
    ...
    if spec.container_image:
        ...
        launcher = (
            f"apptainer exec --nv --bind /shared {spec.container_image} "
            f"jupyter lab --no-browser --ip=$SD_NODE_ADDR --port=$SD_PORT --port-retries=0 "
            f"--ServerApp.root_dir=$SD_USER_ROOT --IdentityProvider.token=\"$JUPYTER_TOKEN\""
        )
    else:
        if spec.conda_env:
            lines += [
    ...
    lines += [
        ...
        f'echo "  ssh -p {gateway_port} -N -L $SD_PORT:$SD_NODE_ADDR:$SD_PORT $USER@{gateway_host}"',
        "echo '  (-N = tunnel only, no shell opens — terminal sitting idle is correct)'",
        f'echo "Then open in browser: http://127.0.0.1:$SD_PORT/lab?token=$JUPYTER_TOKEN"',
        "echo '================================================='",
        "",
        ...
        # own space. cd into SD_USER_ROOT right before launch so both match.
        'cd "$SD_USER_ROOT"',
        launcher,
    ]
    return "\n".join(lines) + "\n"
...
def render_tensorboard_sbatch(spec, folder, logdir, port=6006,
    ...
    if spec.container_image:
        launcher = (
            ...
            f"tensorboard --logdir {logdir} --port $SD_PORT --host $SD_NODE_ADDR"
        )
    else:
        if spec.conda_env:
            lines += [
        ...
        launcher = f"tensorboard --logdir {logdir} --port $SD_PORT --host $SD_NODE_ADDR"
    lines += _NODE_ADDR_SNIPPET + _free_port_snippet(port)
    lines += [
        "echo \'=================================================\'",
        ...
        f'echo "  ssh -p {gateway_port} -N -L $SD_PORT:$SD_NODE_ADDR:$SD_PORT $USER@{gateway_host}"',
        "echo \'  (-N = tunnel only, no shell opens — terminal sitting idle is correct)\'",
        f'echo "Then open: http://127.0.0.1:$SD_PORT"',
        "echo \'=================================================\'",
        "",
        launcher,
    ]
