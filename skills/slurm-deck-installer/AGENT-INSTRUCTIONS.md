# Installing the Slurm Deck — instructions for any AI coding assistant
...
install the Slurm Deck for you. It describes the same guided,
checkpointed process as this repo's Claude Code skill (`skills/slurm-deck-installer`
— use that instead if you're on Claude Code, via `/slurm-deck-installer`).
...
4. **Checkpoint: admin scripts + sudoers.** `install.sh` doesn't install the
   ...
   running them, and always run `sudo visudo -cf /etc/sudoers.d/slurm-deck-admin`
...
6. **Verify the service.** `systemctl status slurm-deck-audit` should be active.

7. **Checkpoint: provision the first admin.** Ask for and confirm the exact
   username, then run `sudo /usr/local/bin/slurm-deck-adduser <username>
