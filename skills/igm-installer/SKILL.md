---
name: igm-installer
description: Use when the user wants to install, set up, or deploy the IIT GPU Manager (IIT Secure SLURM Job Gateway) on a login node / compute node pair. Runs the real install (deploy/install.sh and the steps around it) as a guided, checkpointed process instead of a blind one-shot script.
---

# igm-installer

Guided installation of the IIT GPU Manager onto a fresh (or existing) SLURM
...
account creation depends on this working (`iit-gpu-adduser.sh` needs it).
...
running:
```
sudo /usr/local/bin/iit-gpu-adduser <username> --admin
```
