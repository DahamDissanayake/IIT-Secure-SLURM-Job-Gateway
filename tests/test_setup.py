...
from slurmdeck.ui import BACK_TO_MAIN
...
def test_health_check_passes_when_shared_writable_and_sinfo_ok(tmp_path, monkeypatch):
    ...
    with patch("slurmdeck.slurm.get_partitions", return_value=[MagicMock()]):
        from slurmdeck.setup import check_cluster_health
...
def test_health_check_fails_when_sinfo_returns_nothing(tmp_path, monkeypatch):
    monkeypatch.setenv("NFS_ROOT", str(tmp_path))
    with patch("slurmdeck.slurm.get_partitions", return_value=[]):
        from slurmdeck.setup import check_cluster_health
        from slurmdeck.config import load_config
        ok, messages = check_cluster_health(load_config())
...
def test_smoke_test_script_contains_cuda_check(tmp_path, monkeypatch):
    monkeypatch.setenv("NFS_ROOT", str(tmp_path))
    from slurmdeck.setup import _build_smoke_test_script
    from slurmdeck.config import load_config
...
def test_install_prebuilt_uses_yes_not_removed_force(tmp_path, monkeypatch):
    ...
    from slurmdeck import setup as s
    from slurmdeck.config import load_config
    cfg = load_config()
    ...
    with patch("slurmdeck.setup.questionary.select", return_value=sel), \
         patch("slurmdeck.envbuilder._find_conda", return_value="/shared/miniforge3/bin/conda"), \
         patch("slurmdeck.envbuilder._run_with_progress", side_effect=fake_run_with_progress), \
         patch("slurmdeck.envbuilder._run_pip_with_progress", return_value=(0, [])), \
         patch("slurmdeck.setup._verify_spec_packages", return_value=[]), \
         patch("slurmdeck.setup.auditclient"), \
         patch("slurmdeck.envs._save_venv_registry"), \
         patch("slurmdeck.envs._load_venv_registry", return_value=[]):
...
def test_install_prebuilt_refuses_to_register_incomplete_env(tmp_path, monkeypatch):
    ...
    with patch("slurmdeck.setup.questionary.select", return_value=sel), \
         patch("slurmdeck.envbuilder._find_conda", return_value="/shared/miniforge3/bin/conda"), \
         patch("slurmdeck.envbuilder._run_with_progress", return_value=(0, [])), \
         patch("slurmdeck.envbuilder._run_pip_with_progress", return_value=(0, [])), \
         patch("slurmdeck.setup._verify_spec_packages", return_value=["torch", "transformers"]), \
         patch("slurmdeck.setup.auditclient"), \
         patch("slurmdeck.envs._save_venv_registry", side_effect=fake_save), \
         patch("slurmdeck.envs._load_venv_registry", return_value=[]):
        s._run_install_prebuilt(cfg)
...
def test_run_setup_uses_arrow_select_menu_not_confirm_chain(monkeypatch):
    ...
    with patch("slurmdeck.setup.load_config", return_value=MagicMock()), \
         patch("slurmdeck.setup._run_health_check", return_value=True), \
         patch("slurmdeck.setup.questionary.select", return_value=sel) as select_patch, \
         patch("slurmdeck.setup.questionary.confirm", confirm_mock), \
         patch("slurmdeck.setup._run_model_download", side_effect=lambda cfg: calls.append("model")), \
         patch("slurmdeck.setup._run_smoke_test", side_effect=lambda cfg: calls.append("smoke")):
        s.run_setup()
...
def test_run_setup_back_exits_without_running_steps(monkeypatch):
    ...
    with patch("slurmdeck.setup.load_config", return_value=MagicMock()), \
         patch("slurmdeck.setup._run_health_check", return_value=True), \
         patch("slurmdeck.setup.questionary.select", return_value=sel), \
         patch("slurmdeck.setup._run_model_download", side_effect=lambda cfg: ran.append("x")):
        s.run_setup()
    assert ran == []
