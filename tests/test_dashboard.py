...
def test_get_log_tail_returns_empty_when_file_missing():
    from slurmdeck.dashboard import _get_log_tail
...
def test_find_job_log_returns_none_when_no_file(tmp_path):
    from slurmdeck.dashboard import _find_job_log
...
def test_slurm_time_to_secs_basic():
    from slurmdeck.dashboard import _slurm_time_to_secs
...
def test_queue_all_users_drops_u_flag(monkeypatch):
    ...
    monkeypatch.setenv("SD_DEMO_MODE", "0")
    captured = []
    def fake_run(cmd, **kw):
        captured.append(cmd)
    ...
    with patch("subprocess.run", fake_run):
        from slurmdeck.slurm import queue
...
def test_jobs_table_own_job_shown_bold():
    ...
    from slurmdeck.slurm import QueueEntry
    from slurmdeck.dashboard import _build_jobs_table
...
def test_build_layout_hides_log_for_other_users_job():
    ...
    from slurmdeck.slurm import QueueEntry
    from slurmdeck.dashboard import _build_layout
    from rich.console import Console
...
def test_merged_jobs_calls_queue_with_all_users(monkeypatch):
    ...
    from unittest.mock import patch
    from slurmdeck.dashboard import _merged_jobs
    calls = []
    def fake_queue(**kw):
        calls.append(kw)
        return []
    with patch("slurmdeck.dashboard.queue", fake_queue), \
         patch("slurmdeck.dashboard.filtered_history", return_value=[]):
...
def test_time_remaining_running_job_returns_correct():
    ...
    from slurmdeck.slurm import QueueEntry
    from slurmdeck.dashboard import _time_remaining
...
def test_is_jupyter_job_detects_marker(tmp_path):
    """_is_jupyter_job returns True when .sd-jupyter exists in the job folder."""
    ...
    from slurmdeck.dashboard import _is_jupyter_job
...
def _make_wait_key_env(chars: list[str]):
    ...
    import slurmdeck.dashboard as dash
...
def test_wait_key_returns_regular_char_not_none():
    ...
    with patch.object(dash, '_HAS_TERMIOS', True), \
         patch('slurmdeck.dashboard.select') as mock_sel, \
         patch('slurmdeck.dashboard.sys') as mock_sys:
...
def test_build_status_line_shows_running_job_with_user_and_time():
    ...
    from slurmdeck.slurm import QueueEntry
    from slurmdeck.splash import _build_status_line
    from rich.console import Console
...
def test_resource_status_line_is_the_public_name_for_the_verdict():
    from slurmdeck.splash import resource_status_line
...
def test_pod_blocks_shows_one_glyph_per_pod_occupied_first():
    from slurmdeck.splash import _pod_blocks
...
def test_jupyter_job_without_marker_shows_starting(tmp_path):
    """RUNNING is a scheduler fact, not a usability fact. Until .sd-ready
...
def test_dashboard_offers_connect_key_for_jupyter_jobs():
    ...
    from pathlib import Path
    src = (Path(__file__).resolve().parents[1] / "slurmdeck" / "dashboard.py").read_text()
    assert 'key == "t"' in src
    assert "parse_connect" in src and "render_card" in src


def test_is_ready_job_detects_marker(tmp_path):
    """_is_ready_job returns True when .sd-ready exists in the job folder."""
    job_dir = tmp_path / "notebook_20260530_120000"
    job_dir.mkdir()
    log = job_dir / "slurm-40.out"
    log.write_text("JupyterLab running")
    (job_dir / ".sd-ready").write_text("")
    from slurmdeck.dashboard import _is_ready_job
...
def test_connect_renderable_none_for_other_users_job(tmp_path):
    ...
    from slurmdeck.dashboard import _connect_renderable
    from slurmdeck.slurm import QueueEntry
