...
def test_allowed_commands_list_contains_expected():
    from slurmdeck.shell import ALLOWED_COMMANDS
...
def test_parse_command_splits_correctly():
    from slurmdeck.shell import _parse_command
...
def test_dispatch_blocks_disallowed_command(tmp_path, monkeypatch, capsys):
    ...
    from slurmdeck.shell import _dispatch
...
def test_dispatch_sbatch_accepts_path_inside_jail(tmp_path, monkeypatch):
    ...
    with patch("subprocess.run", side_effect=fake_run), \
         patch("slurmdeck.auditclient.log", return_value=True):
        from slurmdeck.shell import _dispatch
        import importlib, slurmdeck.shell as sh
        importlib.reload(sh)
